...
function _setupMyApp() {
	...
	var html = HtmlService.createHtmlOutput(
		...
		+ 'body{font-family:"Google Sans",Arial,sans-serif;padding:22px 24px 24px;font-size:15px;color:#1f2937;background:#fff}'
		+ '.wiz-progress{display:flex;gap:5px;margin-bottom:18px}'
		+ '.wiz-progress .dot{flex:1;height:5px;border-radius:3px;background:#e5e7eb;transition:background .2s}'
		...
		+ '.hd .ico{font-size:40px;line-height:1;margin-bottom:8px}'
		+ '.hd h2{font-size:20px;font-weight:800;color:#111827;margin-bottom:7px}'
		+ '.hd p{color:#4b5563;font-size:14px;line-height:1.6;max-width:380px;margin:0 auto}'
		+ '.spin{display:inline-block;width:32px;height:32px;border:3px solid #e5e7eb;border-top-color:#2563eb;border-radius:50%;animation:sp .8s linear infinite}'
		+ '@keyframes sp{to{transform:rotate(360deg)}}'
		+ '.center{text-align:center}'
		+ '.btn{width:100%;padding:15px;border:none;border-radius:10px;font-size:15px;font-weight:700;'
		...
		+ '.btn-secondary{background:#fff;color:#374151;border:1px solid #d1d5db;padding:12px}'
		+ '.btn-secondary:hover{background:#f9fafb}'
		+ '.btn-success{background:#16a34a;color:#fff}'
		+ '.btn-link{background:transparent;color:#6b7280;font-weight:600;font-size:13px;padding:8px}'
		+ '.btn:disabled{opacity:.5;cursor:default}'
		+ '.btn-row{display:flex;gap:8px;margin-top:12px}'
		+ '.btn-row .btn{margin-top:0}'
		+ '.steps{margin:0 0 16px}'
		+ '.step{display:flex;gap:12px;padding:11px 0;align-items:flex-start;border-bottom:1px solid #f3f4f6}'
		+ '.step:last-child{border-bottom:none}'
		+ '.num{flex:0 0 28px;width:28px;height:28px;border-radius:50%;background:#dbeafe;color:#1d4ed8;'
		+ 'display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800}'
		+ '.st{font-weight:700;font-size:14px;color:#111827;line-height:1.4;margin-bottom:3px}'
		+ '.sb{color:#4b5563;font-size:13px;line-height:1.6}'
		+ '.sb b{color:#1f2937}'
		+ '.input{width:100%;padding:13px 14px;border:1.5px solid #d1d5db;border-radius:10px;'
		+ 'font-size:14px;outline:none;color:#1f2937;font-family:inherit;margin-bottom:8px}'
		+ '.input:focus{border-color:#60a5fa;box-shadow:0 0 0 3px rgba(96,165,250,.18)}'
		+ '.tip{margin:12px 0 0;padding:13px 15px;border-radius:10px;background:#fef3c7;border:1px solid #fde68a;'
		+ 'color:#78350f;font-size:13px;line-height:1.6}'
		+ '.tip b{color:#92400e}'
		+ '.url-chip{font-size:12px;color:#374151;background:#f3f4f6;border-radius:8px;padding:10px 14px;'
		+ 'word-break:break-all;margin:8px 0 14px;display:block;border:1px solid #e5e7eb}'
		+ '.success-tick{font-size:48px;margin-bottom:10px}'
		+ '.success-h{font-size:20px;color:#166534;font-weight:800;margin-bottom:6px}'
		+ '.success-p{color:#4b5563;font-size:14px;margin-bottom:10px}'
		+ '#msg{min-height:16px;margin-top:10px;font-size:13px;text-align:center;color:#ef4444}'
		+ '#msg.ok{color:#16a34a}'
		+ '</style>'
